...
export function TaskPanel({ task, project, isAdmin, onClose, onDone }: { task: Task; project: Project; isAdmin: boolean; onClose: () => void; onDone: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-sm">
      <motion.aside initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.3, ease: [0.32, 0.72, 0, 1] }} className="surface ml-auto h-full w-full max-w-xl overflow-y-auto rounded-l-lg p-6">
        <div className="flex items-start justify-between gap-4"><div><p className="mb-2 text-sm font-semibold uppercase tracking-wide text-blue-700">Task detail</p><h2 className="text-2xl font-bold tracking-tight">{task.title}</h2><p className="mt-2 leading-6 text-[color:var(--text-secondary)]">{task.description}</p></div><button className="icon-btn" onClick={onClose}><X size={18} /></button></div>
        ...
        {isAdmin ? <button className="btn-danger mt-8 w-full" onClick={() => del.mutate()}><Trash2 size={16} />Delete task</button> : null}
      </motion.aside>
    </div>
  );
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="rounded-lg border border-[color:var(--border)] bg-slate-50 p-4"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[color:var(--text-muted)]">{label}</p>{children}</div>;
}
